...
realisation_recomendations.md - Рекомендации по этапам реализации языка
...
Список последовательности знаков:
;, &|, Знаки сравнения/Методы, Знаки вычисления/Ключи методов  
...
>>, << - потоки.  
container===NoteList()&print(56); Аналог instanceof

#### Поразрядные операции:  
$== - Сравнение строк побитово.  
$* - AND  
$/ - OR  
$% - XOR  
$! - Инверсия  
...
$$'ABCDE'==5 //Подсчет количества элементов  
`$$'ABCDE'[:2]+&$$[3:4]`  
`$$('ABCDE'[:2]+&[3:4])`  
### Функции  
Функции будут обозначены знаком =>:
Функции без this будут обозначены =>>
abc(a, i=0, i<m&i++)=>{}  
Рекурсивные функции:  
abc(a, i, n)=>{  
i<m&print(a)&i++;  
...
abc=>  
-> в c/c++/c# указывает на структуры  
-> или ->> - return.  
.User ==> .Peoples {
...
asm-gas - на нем написан gcc - компилятор c/c++. Имеется возможность вести более оптимизированный код. Возможно посмотреть на реализацию языков программирования на языке ассемблера. Кросплатформенность по сравнению с другими ассемблерами. Встраиваемость в c/c++.   
...
python - добавлены некоторые возможности, которых нет в других языках. Написан на c  
